...
async def incoming_message_f(client, message):
    ...
    if rep_mess:
        ...
        await i_m_sefg.edit("<b>Hey Kawan !</b>\n\n 👁‍🗨 <code>Reply dengan Direct /Torrent Link</code>")
        return
...
async def incoming_youtube_dl_f(client, message):
    ...
    i_m_sefg = await message.reply_text("<code>Prrocessing CMT...🔃</code>", quote=True)
    # LOGGER.info(message)
    ...
    if message.reply_to_message:
        ...
        await i_m_sefg.edit("<b>👁‍🗨 Oops Reply To Link YTDL.</b>")
        return
    if dl_url is not None:
...
async def rename_tg_file(client, message):
    ...
    if len(message.command) > 1:
        ...
        try:
            ...
            if message_to_send != "":
                mention_req_user = (
                    f"<a href='tg://user?id={usr_id}'>🗳 Hey Kawan!! Request Kamu👇</a>\n\n"
                )
                message_to_send = mention_req_user + message_to_send
                message_to_send = message_to_send + "\n\n" + "<b> #UPLOADS\n\n💫 Powered By : @TGFilmZone</b>"
            else:
                message_to_send = "<i>FAILED</i> to upload files CMT. 🗄"
            await message.reply_text(
